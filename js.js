...
if (crypt_multi_transperency) crypt_multi_background_color = 'transparent';
...
crypt_multi_style = 'background: ' + crypt_multi_background_color + '; border: ' + crypt_multi_border_width + 'px solid ' + crypt_multi_border_color + ';' + 'font-family: ' + crypt_multi_font_family + '; font-size: ' + crypt_multi_font_size + '; color: ' + crypt_multi_font_color + ';';
...
for (var i = 0; i < crypt_multi_num_cur; ++i) {
	...
	getPar = eval('(' + httpGet("https://api.cryptonator.com/api/ticker/" + n1.toLowerCase().match(/.*?\(([0-9a-z]+)\)/i)[1] + "-" + n2.toLowerCase().match(/.*?\(([0-9a-z]+)\)/i)[1]) + ')');
	if (getPar.success != false) {
		if (getPar.ticker.price > 1) price = Math.round(getPar.ticker.price * 100) / 100;
		else price = getPar.ticker.price;
...
if (crypt_multi_display_time) document.write('<div style="font-size:70%;opacity:0.6;padding-top:10px;padding-left:10px;">Updated ' + day + ' ' + month + ' ' + year + ' ' + hour + ':' + minutes + ':' + seconds + ' ' + time_zone + '</div>');